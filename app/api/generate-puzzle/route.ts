...
Ensure all words are exactly 5 letters and each consecutive pair differs by only 1 letter.`
    ...
    const response = await fetch('https://apps.abacus.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Generate a ${difficulty} word ladder puzzle for ${date}` }
        ],
    ...
    const puzzleData = JSON.parse(data.choices[0].message.content)
